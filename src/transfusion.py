...
class Transfussion(nn.Module):
    ...
    def forward(
        self,
        modality_tokens:list[tp.Any],   # [(B, Tbf), (B, N, D), (B, Taf)]
        modality_strings:list[str],     # ['text', 'image', 'text']
    ):
        inputs:list[Tensor] = []
        mask_params:list[tuple[int, int]] = []; modality_lengths:list[int] = []

        # Prepare the inputs for the model
        for modality_str, modality_tok in zip(modality_strings, modality_tokens):
            if modality_str == 'text':
                inputs.append(self.embeddings(modality_tok))
                ...
                inputs.append(self.patch_ops(*modality_tok)) # patches, timesteps
        ...
        inputs:Tensor = torch.cat(inputs, dim=1) # (B, T, D)
        outputs = self.model(inputs, self._compute_mask(mask_params)) # (B, T, D)

        # Split the output back into the different modalities
        outputs = torch.split(outputs, modality_lengths, dim=1)
        ...
        for modality_str, modality_tok in zip(modality_strings, outputs):
            if modality_str == "text":
                modality_token_emb.append(self.lm_emb_linear(modality_tok)) # (B, Tbf|Taf, vocab_size)
            elif modality_str == "image":
                modality_token_emb.append(self.img_patch_linear(modality_tok)) # (B, N, (P**2)*C)
            else:
                raise ValueError(f"Unknown modality: {modality_str}")
    ...
    def lm_mode(self, modality_tokens:list[tp.Any], modality_strings:list[str], diff_utils:DiffusionUtils, autocast:torch.autocast):
        ...
        for _ in range(self.txt_maxlens - text_length):
            with autocast:
                modality_token_emb, _ = self(modality_tokens, modality_strings)
            nxt_txt_tok:Tensor = modality_token_emb[-1][:, -1:].argmax(dim=-1) # (B=1, 1)
    ...
    ) -> tuple[list[tp.Any], list[str]]: # (B=1, Tbf), (B=1, C, H, W)
